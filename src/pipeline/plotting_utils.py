...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        ...
        datasets = [
            ("mouse",
             PROJECT_ROOT / "my_results/preproc_results/nucmm/mouse/baseline/diagrams",
             PROJECT_ROOT / "my_results/preproc_results/nucmm/mouse/baseline/plots"),
            ("zebrafish",
             PROJECT_ROOT / "my_results/preproc_results/nucmm/zebrafish/baseline/diagrams",
             PROJECT_ROOT / "my_results/preproc_results/nucmm/zebrafish/baseline/plots"),
        ]
        for name, diag_dir, plot_dir in datasets:
            print(f"[plot] {name}: {diag_dir} -> {plot_dir}")
            plot_dataset(diag_dir, plot_dir, min_persistence=0.04, mode="birth-persistence")
        print("[plot] DONE")
    else:
        main()
